...
def build_weather_features_open_meteo(
    *,
    ...
    """
    Calls Open-Meteo for the flight's local date and picks the hour bin for scheduled departure time.
    ...
    d_temp_max = _safe_get(daily.get("temperature_2m_max"), 0)
    d_temp_min = _safe_get(daily.get("temperature_2m_min"), 0)
    d_precip = _safe_get(daily.get("precipitation_sum"), 0)
    d_wind = _safe_get(daily.get("windspeed_10m_max"), 0)
    d_code = _safe_get(daily.get("weathercode"), 0)
